...
class ProductList { //2.При создании объекта вызывается конструктор
    ...
    getSum() {
        // let s = 0; //Сумма начинается с нуля.
        // this.goods.forEach(item => { //Массив обходим циклом forEach, item - каждый.
        //     s += item.price; //Из каждого элемента берем прайс и суммируем
        // })
        ...
        let res = this.goods.reduce((sum, item) => sum + item.price, 0); //0 - начальное значение первого нашего аргумента sum. item - каждый элемент массива.
        alert(res);
    }
